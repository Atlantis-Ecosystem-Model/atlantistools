...
calculate_spatial_overlap <- function(biomass_spatial, dietmatrix, agemat) {
  ...
  check_df_names(
    biomass_spatial,
    expect = c("species", "agecl", "polygon", "layer", "time", "atoutput")
  )
  check_df_names(
    dietmatrix,
    expect = c(
  ...
  perc_biomass <- agg_perc(
    biomass_spatial,
    groups = c("time", "species", "agecl"),
    out = "perc_bio"
  )
  # Add stanza information
  bio_stanza <- dplyr::left_join(perc_biomass, agemat, by = c("species"))
  bio_stanza$species_stanza <- ifelse(
    bio_stanza$agecl < bio_stanza$age_mat,
    1,
    2
  )
