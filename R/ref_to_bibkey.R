...
ref_to_bibkey <- function(ref_df, bib) {
    ...
    if (nrow(bibkey) > 0) {
      ...
      bibkey <- bibkey[
        purrr::map_lgl(
          bibkey$author,
          ~ sum(
            stringr::str_detect(pattern = ., string = single_ref$author),
            na.rm = T
          ) >
            0
        ),
      ...
      if (nrow(bibkey) > 0 & !is.na(single_ref$title)) {
        ...
        counts <- purrr::map(
          words,
          ~ stringr::str_detect(
            string = single_ref$title,
            stringr::coll(., ignore_case = T)
          )
        )
        ...
        if (length(max_match) == 1 && matches[max_match] > 0.8) {
          return(bibkey$bibkey[max_match])
        }
      } else {
  ...
  purrr::map_chr(
    1:nrow(ref_df),
    ~ single_bibtex(single_ref = ref_df[., ], ref_bib = ref_bib)
  )
}
...
bib_to_df <- function(bib) {
  ...
  if (is.null(single_entries[[mid]])) {
    single_entries[[mid]] <- bib_df[block_ids[mid]:length(bib_df)]
  }
  ...
  get_bibkey <- function(chr) {
    ...
    stringr::str_sub(
      string = out,
      start = stringr::str_locate(string = out, pattern = "\\{")[1] + 1,
  ...
  get_authors <- function(chr) {
    ...
    authors <- purrr::map_chr(
      authors[[1]],
      ~ stringr::str_split_fixed(., pattern = ",", n = 2)[, 1]
    )
  ...
  result <- tibble::tibble(
    bibkey = purrr::map_chr(single_entries, get_bibkey),
