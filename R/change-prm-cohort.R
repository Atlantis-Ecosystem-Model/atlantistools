...
change_prm_cohort <- function(
  ...
  if (length(select_acronyms) == 1 & is.vector(roc)) {
  ...
  if (is.matrix(roc)) {
    roc <- lapply(seq_len(nrow(roc)), function(i) roc[i, ])
  }
  ...
  if (all(sapply(roc, function(x) all(x == 1))) & relative) {
    ...
    if (length(select_acronyms) != length(roc)) {
      stop(
        "Dimensions of select_acronyms and roc do not match. Please supply one row of values per group."
      )
    }
