...
sc_init <- function(
  ...
  if (is.null(pred)) {
    ...
    if (length(acr_age) == 0) {
      stop("Please provide pred as LongName.")
    }
    if (length(acr_age) != length(pred)) {
      stop("Not all predators present in functionalGroups file")
    }
  }
  ...
  vol <- load_init_physics(
    init = init,
    select_variable = "volume",
    bboxes = bboxes
  )
  ...
  if (length(groups_stanza) > 0) {
    ...
    preydens_stanza <- load_init_stanza(
      init = init,
      fgs = fgs,
  ...
  ass_type$ass_type[unlist(sapply(
    c("PHY", "SEAGRAS"),
    grep,
    x = ass_type$grp
  ))] <- "eplant"
...
plot_sc_init <- function(df, mult_mum, mult_c, pred = NULL) {
  ...
  plot <- ggplot2::ggplot(
    ...
    ggplot2::facet_wrap(
      ~ pred + agecl,
      labeller = ggplot2::label_wrap_gen(width = 15, multi_line = FALSE)
    ) +
    ggplot2::scale_fill_gradient(
      "growth real / growth req.",
      low = "red",
      high = "green"
    ) +
