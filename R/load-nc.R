...
load_nc <- function(
  ...
  if (any(!is.element(select_variable, supported_variables))) {
    stop(paste(
      "Only",
      paste(supported_variables, collapse = ", "),
  ...
  if (check_acronyms) {
    ...
    if (length(inactive_groups) >= 1) {
      select_groups <- select_groups[
        !is.element(select_groups, inactive_groups)
      ]
      warning(paste(
        paste(
          "Some selected groups are not active in the model run. Check 'IsTurnedOn' in fgs\n"
        ),
    ...
    if (all(!is.element(select_groups, active_groups))) {
      stop(paste(
        "None of the species selected are active in the model run.",
  ...
  var_names_ncdf <- sapply(
    seq_len(RNetCDF::file.inq.nc(at_out)$nvars - 1),
  ...
  for (i in seq_along(select_groups)) {
    search[[i]] <- c(
      unlist(lapply(
        paste0(select_groups[i], cohorts),
        paste0,
        select_variable
      )), # GroupCohortVariable
      unlist(lapply(
        paste0(select_groups[i], select_variable),
        paste0,
        cohorts
      )), # GroupVariableCohort
      unlist(lapply(
        ...
        select_variable,
        sep = "_"
      )), # GroupCohort_Variable
      unlist(lapply(
        paste(select_groups[i], select_variable, sep = "_"),
        paste0,
        cohorts
      )), # Group_VariableCohort
      unlist(lapply(
        paste(select_groups[i], cohorts, sep = "_"),
        paste,
        select_variable,
        sep = "_"
      )), # Group_Cohort_Variable
      unlist(lapply(
        paste(select_groups[i], select_variable, sep = "_"),
        paste,
        cohorts,
        sep = "_"
      )), # Group_Variable_Cohort
      unlist(paste0(select_groups[i], select_variable)), # GroupVariable
      unlist(paste(select_groups[i], select_variable, sep = "_"))
    ) # Group_Variable
  ...
  final_species <- select_groups[sapply(
    lapply(select_groups, grepl, x = search_clean),
    any
  )]
  ...
  if (length(dim(num_layers)) == 2) {
    if (
      all(apply(num_layers, MARGIN = 1, FUN = function(x) length(unique)) == 1)
    ) {
      num_layers <- num_layers[, 1]
    } else {
      stop(
        "Different numbers of layers per Box. This nc-structure is not supported."
      )
    }
  }
  # add sediment layer!
  ...
  for (i in seq_along(num_layers)) {
    ...
    if (num_layers[i] == 0) {
      ...
      if (!is.null(bboxes) & is.element((i - 1), bboxes)) {
        ...
        layerid[, i] <- c(
          rep(0, times = n_layers - num_layers[i]),
  ...
  layers <- sapply(num_layers[num_layers != 0] - 2, function(x) {
    c(seq(x, from = 0, by = 1), n_layers - 1)
  })
  ...
  if (length(polygons) != length(layers)) {
    stop(
      "Number of polygons and layers do not match. Contact package development team."
    )
  }
  ...
  if (length(at_data3d) >= 1) {
    ...
    result3d <- data.frame(
      species = unlist(lapply(
        ...
        FUN = rep,
        each = length(layers) * n_timesteps
      )),
      agecl = unlist(lapply(
        X = lapply(X = int_fa, FUN = seq, from = 1, by = 1),
        FUN = rep,
        each = length(layers) * n_timesteps
      )),
      polygon = unlist(lapply(
        X = n_timesteps * int_fa,
        FUN = rep,
        x = polygons
      )),
      ...
      time = unlist(lapply(
        X = int_fa,
        FUN = rep,
        x = rep(0:(n_timesteps - 1), each = length(layers))
      )),
  ...
  if (length(at_data2d) >= 1) {
    ...
    result2d <- data.frame(
      ...
      time = unlist(lapply(
        X = int_fa,
        FUN = rep,
        x = rep(0:(n_timesteps - 1), each = length(boxes))
      )),
  ...
  if (length(min_pools) > 0) {
    ...
    if (print_min_pools > 0 & warn_zeros) {
      warning(paste0(
        round(print_min_pools / dim(result)[1] * 100),
        "% of ",
        select_variable,
        " are true min-pools (0, 1e-08, 1e-16)"
      ))
    }
