...
combine_groups <- function(
  ...
  group_col,
  groups = names(data)[
    !is.element(names(data), c("atoutput", "time", group_col))
  ],
  combine_thresh = 15
) {
  if (combine_thresh <= 1) {
    stop("Minimum value for 'combine_thresh' is 2.")
  }
  ...
  if (nrow(low_contrib) != 0) {
    ...
    if (any(dim(data) != dim(new_data))) {
      stop("Please contact package development Team.")
    }
    ...
    comb_data <- agg_data(
      new_data,
      groups = names(data)[!is.element(names(data), "atoutput")],
      fun = sum
    )
    return(comb_data)
  } else {
