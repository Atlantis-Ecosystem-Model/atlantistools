...
load_init <- function(init, vars) {
  ...
  if (n_timesteps != 1) {
    stop("More than 1 timestep! init was not an initial conditions file.")
  }
  ...
  var_names_ncdf <- sapply(
    seq_len(RNetCDF::file.inq.nc(read_nc)$nvars - 1),
  ...
  if (length(wrong_var) >= 1) {
    stop(paste(
      "Variable",
      paste(wrong_var, collapse = " "),
      "not found in init file."
    ))
  }
  ...
  convert2d <- function(mat, layerid, n_boxes) {
    if (!(is.matrix(mat) & length(dim(mat)) == 2)) {
      stop(
        "Wrong data format. Variable is not stored as 2d data in initial file."
      )
    }
    data.frame(
      atoutput = as.vector(mat),
  ...
  convert1d <- function(vec, n_boxes) {
    if (!(is.vector(vec) & length(vec) == n_boxes)) {
      stop(
        "Wrong data format. Variable is not stored as 1d vector in initial file."
      )
    }
    data.frame(
      atoutput = as.vector(vec),
      polygon = 0:(n_boxes - 1),
      stringsAsFactors = FALSE
    )
  }
  ...
  if (all(at_dim == 2)) {
    df_list <- lapply(at_data, convert2d, layerid, n_boxes)
  }
  #  if (all(at_dim == 1)) df_list <- lapply(at_data, convert1d, n_boxes)
  # vectors are dimensionless in R. Would be nice if dim returned a 1!
  if (all(at_dim == 0)) {
    df_list <- lapply(at_data, convert1d, n_boxes)
  }
  if (length(unique(at_dim)) > 1) {
    stop(
      "Vars are stored in different dimensions. Please, either pick only 2d or 1d data."
    )
  }

  # Data extracted for every variable?
  if (length(vars) != length(df_list)) {
    stop("Starnge ncdf extraction. Please contact package development Team.")
  }
...
get_layerid <- function(num_layers, max_layer, n_boxes) {
  ...
  if (length(unique(sapply(wc, length))) != 1) {
    stop("Layers inconsistent. Contact package development Team.")
  }
