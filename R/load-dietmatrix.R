...
load_dietmatrix <- function(
  ...
  acr <- fgs_data$Code[
    fgs_data[, names(fgs_data)[
      names(fgs_data) %in% c("isPredator", "IsPredator")
    ]] ==
      1
  ]
  agecl <- fgs_data$NumCohorts[
    fgs_data[, names(fgs_data)[
      names(fgs_data) %in% c("isPredator", "IsPredator")
    ]] ==
      1
  ...
  diet_strings <- c(
    ...
    as.vector(t(outer(
      X = as.vector(outer(X = paste0(pstring, 1:2), Y = coh10, FUN = paste0)),
      Y = 1:2,
      FUN = paste0
    ))),
  ...
  dietmatrix <- extract_prm_cohort(
    prm_biol = prm_biol,
    variables = diet_strings
  )
  if (length(unique(sapply(dietmatrix, length))) != 1) {
    stop(
      "Number of entries in dietmatrix are not equal. Check your dietmatrix."
    )
  } else {
  ...
  prey_stanza <- suppressWarnings(as.integer(substr(
    rownames(dietmatrix),
    start = nchar(pstring) + 1,
    stop = nchar(pstring) + 1
  )))
  prey_stanza[is.na(prey_stanza)] <- 2
  pred_stanza <- suppressWarnings(as.integer(substr(
    rownames(dietmatrix),
    start = nchar(rownames(dietmatrix)),
    stop = nchar(rownames(dietmatrix)) + 1
  )))
  pred_stanza[is.na(pred_stanza)] <- 2
  pred <- c(rep(coh10, each = 4), coh1, rep(coh2, each = 2))
  if (length(pred) != nrow(dietmatrix)) {
    stop("Incomplete rows in diet data.")
  }

  # Extract preys. Add sediment prey to Carrion and Det groups!
  acronyms <- get_acronyms(fgs = fgs)
  car_det <- fgs_data$Code[grep(
    pattern = "(\\_DET|CARRION)",
    x = fgs_data[, names(fgs_data) %in% c("InvertType", "GroupType")]
  )]
  prey <- c(acronyms, paste0(car_det, "sed"))
  if (length(prey) != ncol(dietmatrix)) {
    stop("Incomplete columns in diet data")
  }
...
write_diet <- function(dietmatrix, prm_biol, save_to_disc = TRUE) {
  ...
  if (any(lags > 3)) {
    ...
    warning(paste0(
      "Multiple linesbreaks present in ",
      prm_biol,
  ...
  if (length(dm_ids) < (2 * nrow(dietmatrix))) {
    ...
    avails <- apply(
      X = dietmatrix[, 5:ncol(dietmatrix)],
      FUN = paste,
      MARGIN = 1,
      collapse = "\t"
    )
