...
plot_diet_bec_dev <- function(
  ...
  check_df_names(
    data = data,
    expect = c("time", "atoutput", "prey", "pred"),
    optional = c("agecl", "stanza")
  )
  ...
  for (i in seq_along(grobs)) {
    heading <- grid::textGrob(
      paste("Indication of feeding interaction:", species[i]),
