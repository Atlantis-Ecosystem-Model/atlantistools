...
#'
#'   grobs <- mapply(gridExtra::arrangeGrob, plots_pred, plots_prey, MoreArgs = list(heights = grid::unit(c(0.7, 0.3), units = "npc")))
#'
#'   return(grobs)
#' }
#'
...
#'
#' test <- combine_groups(mort_agecl,
#'                        group_col = "prey",
#'                        groups = c("time", "pred", "agecl"),
#'                        combine_thresh = 0.03)
#'
#' ggplot2::ggplot(data = test, ggplot2::aes(x = time, y = prey, fill = atoutput)) +
#'   ggplot2::geom_tile() +
#'   ggplot2::scale_fill_gradientn(colours = rainbow(7), name = "Diet [%]") +
#'   ggplot2::facet_wrap(~ pred + agecl, ncol = 10, scale = "free")
#'
#'
#'
