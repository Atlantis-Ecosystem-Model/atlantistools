...
```{r}
check_dz <- result$dz %>% 
  left_join(result$nominal_dz, by = c("polygon", "layer")) %>% 
  mutate(check_dz = atoutput.x / atoutput.y) %>% 
  filter(!is.na(check_dz)) # remove sediment layer

plot <- plot_line(check_dz, x = "time", y = "check_dz", wrap = "polygon", col = "layer")
plot <- update_labels(plot, list(x = "Time [years]", y = expression(dz/nominal_dz)))
```
...
```{r}
plot_line(result$growth_rel_init, y = "gr_rel")
...
# Spatial Plots 1
```{r, echo=FALSE, results = 'asis', fig.width = 14, fig.height = 9}
plots <- plot_spatial_box(result$biomass_spatial_stanza, bgm_as_df = convert_bgm(dir = dir, bgm = bgm), timesteps = 7)
for (i in seq_along(plots)) {
...
# Spatial Plots 2
```{r, echo=FALSE, results = 'asis', fig.width = 14, fig.height = 9}
plots <- plot_spatial_ts(result$biomass_spatial_stanza, bgm_as_df = convert_bgm(dir = dir, bgm = bgm), vol = result$vol)
for (i in seq_along(plots)) {
  cat(paste0("## Spatial Plot ", i, ": ", names(plots)[i]), sep = "\n")
  gridExtra::grid.arrange(plots[[i]])
  cat("\n\n")
