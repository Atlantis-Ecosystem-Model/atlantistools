...
Data about feeding interactions can be visualised with `plot_diet_bec_dev`. The data originates from `SpecPredMort.txt`
which gives an indication about feeding interactions. Please note that these values do not represent realised diets within
the model. If you want to visualise realised diets you need oto switch to the trunk version of ATLANTIS and use
`plot_diet` instead.
...
feeding_plots <- plot_diet_bec_dev(data = setas$diet_specmort, wrap_col = "agecl", combine_thresh = 15)
...
```{r}
extract_prm(dir = d, prm_biol = "VMPA_setas_biol_fishing_New.prm", variable = "KWRR_FPS")
extract_prm(dir = d, prm_biol = "VMPA_setas_biol_fishing_New.prm", variable = "KWSR_FPS")
```
