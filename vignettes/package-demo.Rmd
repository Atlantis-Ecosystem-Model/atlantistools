...
During preprocessing of atlantis simulations data is only extracted from the nc files.
Since information about feeding interactions is stored in the `DietCheck.txt` files the
data has to be read in first using `load_dietcheck`. This may change in later stages of the
package by adding `load_dietcheck` to `preprocess`.
...
Please note that recruitment in Atlantis differs with recruitment in most other models due to changes of recruitment
ages (which is generally age 0-3 in stock assessment models). Thanks @ Isaac for pointing that out.
Both recruiment and ssb is stored in `*.txt` files like the `DietCheck.txt`. Therefore the data has to be read in first.
...
As the Setas model comes without recruitment data I will create the data randomly.
...
plot_rec(data = rec, ex_data)
```

## Compare your model run with other models/data!

I simply add some noise to the biomass timeseries to show the resulting plot.
...
change the existing valu by multilpying with a factor or set a new absolut value. In our case
...
You can you `extract_prm` to check if the resulting *.prm is correct. However, I checked the function
thouroghly you so you would normally set `save_to_disc` to `TRUE` and overwrite your existing
parameter file with the new one. Please make sure to backup your original model.
...
In case you want to change the parameter values for cohort structured groups whose paramter expect multiple values
(E.g. Clearance rate and mum for fish groups) please use `change_prm_cohrt`.
