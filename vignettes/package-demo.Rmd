...
If you want to work with external software in further steps (E.g. Excel) you should convert the dataframes to `*.txt` or `*.csv` files.
...
1. All your model files are stored in one directory. The files can now be accesed via their name. The directory can be set/passed in two ways:
...
2. In case you are using different folders for generic model files and output files the directory can be set as described above. However you need to
...
Please note that the plots will look much nicer if the simulation period is elongated.
...
Both recruiment and ssb is stored in `*.txt` files like the `DietCheck.txt`. Therefore the data has to be read in first with `load_rec`.
...
thouroghly so you would normally set `save_to_disc` to `TRUE` to overwrite your existing
