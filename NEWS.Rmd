...
mgN is converted to tonnes based on the stettings in the biol.prm file. Simulation time steps
are converted to time in years based on output timesteps given in run.prm. Group names are unified to
the column 'LongName' in the functionalGroups.csv. All these transformations are applied in every
...
group by a predator/predatorageclass combination for each timestep and polygon. Consumption data is extracted from output[...]PROD.nc. Age based groups are stored as "Eat_" non age based groups as "Grazing_". Units are mg N m^-3 d^-1.
...
for models based on the trunk code. Units are % diet contribution. To calculate the consumed biomass by preytype
...
* `calculate_spatial_overlap` The spatial overlap between predator and prey groups is calculated as
...
Secondly
Dataframe in first listentry gives pred, pred agecl, prey, preyageclass
...
This will allow you to easily track unrealistically high changes throughout the simulation.
* `plot_diet` updated the way ``plot_diet` works. The new version of `plot_diet` only works with trunk models. Please use `plot_diet_bec_dev` for 'bec_dev' models. `plot_diet()` now displays the realised diet contribution (both per predator and
prey perspective) based an the amount of consumed biomas. Please note, `plot_diet_bec_dev` does not work with DietCheck.txt anymore. Please use SpecPredMort.txt instead. It should be noted that the resulting plot only gives an indication of the 
