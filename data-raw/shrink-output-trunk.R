  ...
  paste0(
    ...
    c(
      sort(as.vector(outer(
        as.vector(outer(coh_groups, 1:10, FUN = paste0)),
        c("Nums", "ResN", "StructN"),
        FUN = paste,
        sep = "_"
      ))),
...
# Replace fill values
update_vars <- keep_vars[unlist(purrr::map(
  c("ResN", "StructN"),
  ~ which(grepl(pattern = .x, x = keep_vars))
))]
fill <- paste0(stringr::str_sub(update_vars, start = 2), ":_FillValue")

ids <- purrr::map_int(fill, grep, x = new_init)

filler <- as.numeric(stringr::str_replace(
  stringr::str_split_fixed(new_init[ids], pattern = " = ", n = 2)[, 2],
  pattern = " ;",
  replacement = ""
))
...
writeLines(
  new_init,
  con = file.path("new", paste0(paste0(basename(file_gen), ".cdf")))
)
...
writeLines(
  new_init,
  con = file.path("new", paste0(paste0(basename(file_prod), ".cdf")))
)
