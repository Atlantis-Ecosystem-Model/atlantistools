...
get_diet_fishbase <- function(fish, mirror = "se") {
  ...
  result <- purrr::map_if(
    diet_urls,
    ids,
    ~ xml2::read_html(paste0("http://www.fishbase.", mirror, .))
  ) %>%
  ...
  if (any(!ids)) {
    nas <- tibble::as_tibble(do.call(
      rbind,
      purrr::map(fish[!ids], ~ c(rbind(rep(NA, length(df_names) - 1)), .))
    )) %>%
      purrr::set_names(., df_names)
  ...
  if (all(is.na(diet_df$ref_id))) {
    stop(
      "None of the species have information about diets Add additional species."
    )
  } else {
    return(diet_df)
  }
