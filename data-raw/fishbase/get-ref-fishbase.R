...
get_ref_fishbase <- function(ref_id, mirror = "se") {
  ...
  ref <- purrr::map_chr(
    ...
    ~ paste0(
      "http://www.fishbase.",
      mirror,
      "/References/FBRefSummary.php?ID=",
      .
    )
  ) %>%
  ...
  clean_ref$year <- as.integer(purrr::map_chr(
    purrr::map_if(clean_ref$ref, ~ !is.na(.), str_split_twice),
    1
  ))
  clean_ref$author <- stringr::str_sub(
    clean_ref$ref,
    end = stringr::str_locate(
      string = clean_ref$ref,
      pattern = as.character(clean_ref$year)
    )[, 1] -
      3
  )
  clean_ref$title <- stringr::str_sub(
    clean_ref$ref,
    start = stringr::str_locate(
      string = clean_ref$ref,
      pattern = paste(clean_ref$author, clean_ref$year, sep = ", ")
    )[, 2] +
      3
  )
