...
test_that("test change_prm", {
  expect_equal(
    extract_prm(
      prm_biol = prm_old,
      variables = paste("KWRR", "FPS", sep = "_")
    ),
    0.075
  )
  ...
  expect_equal(
    extract_prm(
      prm_biol = prm_old,
      variables = paste("KWRR", "FVS", sep = "_")
    ),
    7000
  )
  ...
  expect_equal(
    which(prm_old_read != prm_new),
    c(
      scan_prm(
        chars = prm_old_read,
        variable = paste("KWRR", "FPS", sep = "_")
      ),
...
# "mL", "mQ"
pos <- sapply(
  paste(supported_parameters, "FPS", sep = "_"),
  scan_prm,
  chars = prm_old_read
)
...
test_that("test scan_prm & extract_prm", {
  expect_error(
    scan_prm(chars = c("# KWRR_FVS 15"), variable = "KWRR_FVS"),
    "Variable KWRR_FVS always outcommented."
  )

  expect_error(
    scan_prm(
      chars = c("# KWRR_FVS 15", "# KWRR_FVS 25"),
      variable = "KWRR_FVS"
    ),
    "Variable KWRR_FVS always outcommented."
  )
  ...
  expect_error(
    scan_prm(chars = c("KWRR_FVS 15", "KWRR_FVS 25"), variable = "KWRR_FVS"),
    "Variable KWRR_FVS found multiple times."
  )
...
test_that("test flags embedded in other flags", {
  expect_equal(
    prm_dummy[scan_prm(chars = prm_dummy, variable = "WSK_age_mat")],
    dummy[1]
  )
  expect_equal(
    prm_dummy[scan_prm(chars = prm_dummy, variable = "LSK_age_mat")],
    dummy[2]
  )
  expect_equal(
    prm_dummy[scan_prm(chars = prm_dummy, variable = "SK_age_mat")],
    dummy[3]
  )
  expect_equal(
    prm_dummy[scan_prm(chars = prm_dummy, variable = "jmL_COD")],
    dummy[4]
  )
  expect_equal(
    prm_dummy[scan_prm(chars = prm_dummy, variable = "mL_COD")],
    dummy[5]
  )
  expect_equal(
    prm_dummy[scan_prm(chars = prm_dummy, variable = "mum_COD")],
    dummy[6]
  )
  expect_equal(
    prm_dummy[scan_prm(chars = prm_dummy, variable = "crit_mum_COD")],
    dummy[7]
  )
})
