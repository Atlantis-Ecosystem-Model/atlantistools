...
test_that("test agg_data with reference data for numbers", {
  expect_is(
    agg_data(data = ref_nums, groups = c("species", "agecl"), fun = sum),
    "data.frame"
  )
  expect_equal(
    dim(agg_data(data = ref_nums, groups = c("agecl"), fun = mean))[1],
    10
  )
  expect_equal(
    dim(agg_data(data = ref_nums, groups = c("agecl"), fun = sum))[1],
    10
  )
  expect_equal(
    agg_data(
  ...
  expect_equal(
    names(perc),
    c("species", "polygon", "layer", "time", "atoutput", "test")
  )
  expect_equal(
    agg_data(perc, col = "test", groups = "species", fun = sum)$atoutput,
    rep(1, times = 5)
  )
})
