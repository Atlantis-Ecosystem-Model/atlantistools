...
file_ending(
  "C:/Users/ryanmorse/Documents/GitHub/atneus_RM/RMinit_2.csv",
  ending = "csv"
)

test_that("test different file endings", {
  expect_error(
    file_ending("C:/Users/ryanmorse/Documents/GitHub/atneus_RM/RMinit_2.csv"),
    "does not end in nc"
  )
})
...
test_that("test result of multisplit calls", {
  ...
  expect_equal(
    length(r2),
    length(unique(ref_nums$species)) * length(unique(ref_nums$agecl))
  )
  ...
  expect_equal(
    length(r3),
    length(unique(ref_nums$species)) *
      length(unique(ref_nums$agecl)) *
      length(unique(ref_nums$layer))
  )
